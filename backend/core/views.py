...
class ERPViewSet(viewsets.ModelViewSet):
    ...
    def generate_pdf(self, request, pk=None):
        ...
        if post.post_type == "Supply":
            role_rule = "Supply: Post owner is Provider, booking user is Receiver"
        else:
            role_rule = "Demand: Applicant user is Provider, post owner is Receiver"

        snapshot = erp.configuration_snapshot or {}

        def text_row(canvas_obj, text, x, y):
            canvas_obj.drawString(x, y, str(text)[:120])

        def ensure_space(canvas_obj, y, minimum=70):
            if y < minimum:
                canvas_obj.showPage()
                return 800
            return y
        ...
        pdf.drawString(50, 780, f"Post: {post.post_name}")
        pdf.drawString(50, 760, f"Post Type: {post.post_type}")
        pdf.drawString(50, 740, f"Post Owner: {user_label(owner)}")
        pdf.drawString(50, 720, f"Provider: {user_label(provider)}")
        pdf.drawString(50, 700, f"Receiver: {user_label(receiver)}")
        pdf.drawString(50, 680, f"Category (for viewer): {erp.category}")
        pdf.drawString(50, 660, f"Stage: {erp.stage}")
        pdf.drawString(50, 640, f"Total Cost: {erp.total_cost}")
        pdf.drawString(50, 620, f"Generated By: {user_label(actor)}")
        pdf.drawString(50, 600, role_rule)

        y = 575
        pdf.drawString(50, y, "Finalized Configuration (from ManagePost)")
        y -= 18

        if snapshot:
            generated_at = snapshot.get("generated_at") or "N/A"
            text_row(pdf, f"Configured At: {generated_at}", 50, y)
            y -= 14

            totals = snapshot.get("totals") or {}
            text_row(pdf, f"Expertise Total: {totals.get('expertise', 0):.2f}", 50, y)
            y -= 14
            text_row(pdf, f"Service Total: {totals.get('services', 0):.2f}", 50, y)
            y -= 14
            text_row(pdf, f"Product Total: {totals.get('products', 0):.2f}", 50, y)
            y -= 14
            text_row(pdf, f"Grand Total: {totals.get('grand', float(erp.total_cost)):.2f}", 50, y)
            y -= 18

            sections = [
                ("Expertise", snapshot.get("expertise") or []),
            ...
            for title, rows in sections:
                if not rows:
                    continue
                y = ensure_space(pdf, y)
                text_row(pdf, f"{title}:", 50, y)
                y -= 14
                for row in rows[:20]:
                    y = ensure_space(pdf, y)
                    row_text = (
                        f"- {row.get('name', '-')}: unit_cost={row.get('unit_cost', 0)} "
                        f"qty={row.get('quantity', 0)} duration={row.get('duration', 0)} "
                        f"line_total={row.get('line_total', 0)}"
                    )
                    text_row(pdf, row_text, 60, y)
                    y -= 12
                y -= 6
        else:
            text_row(pdf, "No finalized ManagePost snapshot saved yet.", 50, y)
